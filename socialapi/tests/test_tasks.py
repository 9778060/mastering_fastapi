import httpx
import pytest
from ..tasks import APIResponseError, send_simple_message, _generate_cute_creature_api, generate_and_add_to_post
...
async def test_send_simple_message(mock_httpx_client):
    await send_simple_message("test@test.com", "Test Subject", "Test Body")
...
async def test_generate_and_add_to_post_success(
    mock_httpx_client, created_post: list[dict], confirmed_user: dict, db: Database
):
    json_data = {"output_url": "https://example.com/image.jpg"}
    ...
    query = post_table.select().where(post_table.c.id == created_post[0]["id"])
